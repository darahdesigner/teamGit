...
const Ticketmaster = (props) => {
    ...
    const handleClick = () => {
        fetch(`https://app.ticketmaster.com/discovery/v2/events.json?&apikey=pfyF9lEGdQN8wKexFoe6ulE5qcZigjRG&size=5&sort=date,asc&radius=100&latlong=${props.lat},${props.lng}`)
        .then(res => res.json())
        .then(res =>{
            console.log(res)
          ...
          setD(res._embedded.events[0].dates.start.localDate)    
          setD1(res._embedded.events[1].dates.start.localDate)    
          setD2(res._embedded.events[2].dates.start.localDate)    
          setD3(res._embedded.events[3].dates.start.localDate)    
          setD4(res._embedded.events[4].dates.start.localDate) 
        //   setPhoto(res._embedded.events[0].images[6].url)   
        })
    };
